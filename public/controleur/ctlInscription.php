    ...
    public function regMember()
    {
        ...
        if (empty($email) || empty($username) || empty($mdp) || empty($mdp_confirm)) {
            if ($_SESSION['lang'] === 'ENG')
                $message .= "<span class='text-red-500'>*Please complete all fields</span><br>";
            else
            ...
            if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL))
                // EMAIL invalide
                if ($_SESSION['lang'] === 'ENG')
                    $message .= "<span class='text-red-500'>*Please put a valid email</span><br>";
                else
            ...
            if ($_POST['mdp_confirm'] != $_POST['mdp']) {
                ...
                if ($_SESSION['lang'] === 'ENG')
                    $message .= "<span class='text-red-500'>*Password does not match</span><br>";
                else
        ...
        if (empty($message)) {
            if ($this->member->verifUsernameBDD($username) == TRUE) {
                ...
                if ($_SESSION['lang'] === 'ENG')
                    $message .= "<span class='text-red-500'>*Username already taken</span><br>";
                else
                ...
                if ($_SESSION['lang'] === 'ENG')
                    $message .= "<span class='text-red-500'>*Email already taken</span><br>";
                else
